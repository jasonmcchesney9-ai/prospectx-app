...
export default function TeamImportPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Pass team override so all imported players get assigned to this team
      const { data } = await api.post<ImportPreview>(
        `/import/preview?team_override=${encodeURIComponent(teamName)}`,
        formData
      );
      setPreview(data);
  ...
  return (
    <ProtectedRoute>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        >
          <ArrowLeft size={14} /> Back to {teamName}
        </Link>

        <h1 className="text-2xl font-bold text-navy mb-1">Import Roster — {teamName}</h1>
        <p className="text-sm text-muted mb-6">
          Upload a CSV or Excel file to add players to {teamName}. All imported players will be automatically assigned to this team.
        </p>
        ...
        {step === "upload" && (
          <div className="bg-white rounded-xl border border-border p-8 text-center">
            ...
            <p className="text-sm text-muted mb-6 max-w-md mx-auto">
              Upload a CSV or Excel file (.xlsx) with roster data. Required columns: <strong>First Name</strong>, <strong>Last Name</strong>.
              Optional: Position, DOB, Shoots, Height, Weight, GP, G, A, P, PIM.
            </p>
            <p className="text-xs text-teal/80 mb-4 max-w-md mx-auto bg-teal/5 rounded-lg px-4 py-2">
              All players will be assigned to <strong>{teamName}</strong> automatically.
            </p>
...
Jake,Wilson,D,2005-11-05,R,25,2,8,10`}
              </pre>
            </div>
          </div>
        )}
                  ...
                  {preview.duplicates.map((dup: ImportDuplicate) => (
                    <div key={dup.row_index} className="p-4">
                      <div className="flex items-start justify-between gap-4 mb-3">
                        <div>
                          <p className="text-sm font-semibold text-navy">CSV: {dup.csv_name}</p>
